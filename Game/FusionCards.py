...
class Popup:
    def __init__(self, x, y, width, height, title, text, color):
        ...
        self.exit_button = pygame.Rect(self.rect.x + self.header_height/4, self.rect.y + self.header_height/4, 20, 20)
    ...
    def draw(self, screen):
        if self.is_visible:
            ...
            # Main Box
            pygame.draw.rect(screen, self.body_color, self.rect, border_radius=15)
            pygame.draw.rect(screen, (255, 255, 255), self.rect, 2, border_radius=15) # Border

            # Render Wrapped Text
            lines = self.get_wrapped_lines()
            for i, line in enumerate(lines):
                line_surf = self.font.render(line.strip(), True, (255, 255, 255))
                # Position each line based on its index
                line_y = self.rect.y + self.header_height + self.padding + (i * self.line_spacing)
        
                
                # Check to make sure we don't draw outside the box bottom
                if line_y < self.rect.bottom - self.padding:
                    screen.blit(line_surf, (self.rect.x + self.padding, line_y))
            ...
            pygame.draw.line(screen, (255, 255, 255), 
                             ...
                             (self.rect.right -1, self.rect.y + self.header_height), 2)
            ...
            x_margin = 5
            pygame.draw.line(screen, (255, 255, 255), 
                         (self.exit_button.x + x_margin, self.exit_button.y + x_margin),
                         (self.exit_button.right - x_margin, self.exit_button.bottom - x_margin), 3)
            # Line 2: Top-right to bottom-left
            pygame.draw.line(screen, (255, 255, 255), 
                         (self.exit_button.right - x_margin, self.exit_button.y + x_margin),
                         (self.exit_button.x + x_margin, self.exit_button.bottom - x_margin), 3)
...
class Discoveries(Popup):
    def __init__(self, element):
        ...
        super().__init__((1000-WIDTH)/2, (800-HEIGHT)/2, WIDTH, HEIGHT,
                          atoms_name[index], atoms_info[index], atoms_color[index])

print(Atom("H-2", None, None, None).identity)
